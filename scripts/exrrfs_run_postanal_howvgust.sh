...
#-----------------------------------------------------------------------
#
# Regridding the firstguess of ocean Significant Wave Height (howv)
# and 10-m wind gust (gust) fields (in surface analysis file) for 3DRTMA
# Note; regridding the data from the Extended Schmidt Gnomonic (ESG) 
# grid to the Rotated Latlon (RLL) grid
#
#-----------------------------------------------------------------------
#
if [[ "${NET}" == "RTMA"* ]] && [[ "${BKTYPE}" -eq 0 ]] ; then

   #==== regrdding Ocean Significant Wave Height (HOWV) from ESG grid to RLL grid ====#
   export varname="howv"
   export varname_grb="HTSGW"
   #-- testing if howv exists in firstguess sfc_data.nc (with ncdump)
   i_found_howv=0
   i_found_howv=$(ncdump -h ${bkpath}/sfc_data.nc | grep -i "${varname}" | wc -l) 
   if [[ "${i_found_howv}" -eq 1  ]] && [[ "${DO_HOWV^^}" == "TRUE" ]] && [[ "${MACHINE,,}" == "wcoss2" ]] ; then
      ...
      # if using the incremental interpolation, linking the firstguess file (also on ESG grid)
...
EOF
      ...
      # running executable for regridding
      ...
      export err=$?; err_chk
      mv errfile errfile_regrid_esg2rll_${varname}
...
#        err_exit "Could NOT find grib2 template file for ${varname}. Abort ..."
      ...
      RUN_HOWV="FALSE"
      print_info_msg "VERBOSE" "Could NOT find the ${varname} in analysis file (sfc_data.nc).  \
                      Skipping the conversion from ESG to RLL for ${varname}."
   fi

   #==== regrdding 10-m Wind Gust (GUST) from ESG grid to RLL grid ====#
   export varname="gust"
   export varname_grb="GUST"
...
EOF
      ...
      export err=$?; err_chk
      mv errfile errfile_regrid_esg2rll_${varname}
      print_info_msg "VERBOSE" "Successfully regridding the analysis of 10-m wind gust (gust) \
                     from ESG grid to RLL grid. "
      #---- using wgrib2 to convert the netcdf-format howv analysis file to grib2 format (for step prdgen)
      #-- Note: need the grib2 template file (using the original firstguess grib2 file on RLL grid)
      bkpath_howvgust=${cycle_dir}/process_howvgust
